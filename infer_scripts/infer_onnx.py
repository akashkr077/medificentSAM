...
_visualize_spec = importlib.util.spec_from_file_location(
    "medficientsam_visualize",
    Path(__file__).resolve().parents[1] / "src" / "utils" / "visualize.py",
)
_visualize = importlib.util.module_from_spec(_visualize_spec)
_visualize_spec.loader.exec_module(_visualize)
save_comparison_outputs = _visualize.save_comparison_outputs

if save_overlay:
    visualize_output = _visualize.visualize_output
...
def infer_2D(img_npz_file: str, encoder: SamEncoder, decoder: SamDecoder):
    ...
    save_comparison_outputs(npz_data["imgs"], segs, output_dir, npz_name)
...
def infer_3D(img_npz_file: str, encoder: SamEncoder, decoder: SamDecoder):
    ...
    np.savez_compressed(output_dir / npz_name, segs=segs)
    z = segs.shape[0] // 2
    save_comparison_outputs(npz_data["imgs"][z], segs[z], output_dir, npz_name)
