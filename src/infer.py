...
_cache_dir = Path(tempfile.gettempdir()) / "medficientsam-cache"
...
torch.serialization.add_safe_globals([functools.partial, torch.optim.AdamW])
import hydra
import rootutils
...
from src.utils.visualize import save_comparison_outputs, visualize_output
...
def resolve_device(requested_device: str) -> torch.device:
    ...
    if device_type == "cuda" and not torch.cuda.is_available():
        if torch.backends.mps.is_available():
            fallback = torch.device("mps")
        else:
            fallback = torch.device("cpu")
        log.warning(
            f"Requested device '{requested_device}' is unavailable because this "
            f"PyTorch build has no CUDA support. Falling back to '{fallback}'."
        )
        return fallback

    if device_type == "mps" and not torch.backends.mps.is_available():
        log.warning("Requested device 'mps' is unavailable. Falling back to 'cpu'.")
        return torch.device("cpu")

    return torch.device(requested_device)


def configure_dataloader_for_device(cfg: DictConfig, device: torch.device) -> None:
    if device.type == "cuda":
        return
    ...
    log.warning(
        f"Using '{device}' inference, so DataLoader multiprocessing and pinned "
        "memory were disabled."
    )
...
def infer_2D(model: BaseSAM, data, device, output_dir: Path, save_overlay: bool):
    ...
    save_comparison_outputs(data["original_image"], segs, output_dir, npz_name)
...
def infer_3D(model: BaseSAM, data, device, output_dir: Path, save_overlay: bool):
    ...
    np.savez_compressed(output_dir / "npz" / npz_name, segs=segs)
    z = segs.shape[0] // 2
    save_comparison_outputs(data["original_image"][z], segs[z], output_dir, npz_name)
...
def infer(cfg: DictConfig):
    ...
    for data in tqdm(dataloader):
        ...
        if data["image_type"] == "2D":
            infer_2D(model, data, device, output_dir, cfg.save_overlay)
        elif data["image_type"] == "3D":
            infer_3D(model, data, device, output_dir, cfg.save_overlay)
        else:
